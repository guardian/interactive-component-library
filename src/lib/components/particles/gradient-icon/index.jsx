...
import { useGradients, setGradients } from './gradientStore'
...
export const GradientIcon = (props) => {
  ...
  const gradientId = `gv-gradient-def-${previous}-${next}`
  const gradients = useGradients()

  useEffect(() => {
    setGradients((current) => {
      current[gradientId] = { id: gradientId, ...props }
      return current
    })
  }, [gradientId, props])

  useEffect(() => {
    let container = document.getElementById('gv-gradient-defs')
    if (!container) {
      container = document.createElement('div')
      container.id = 'gv-gradient-defs'
      document.body.prepend(container)
    }

    render(<GradientDefs gradients={Object.values(gradients)} />, container)
  }, [gradients])
...
function GradientDefs({ gradients }) {
  return (
    <svg width="24" height="11" viewBox="0 0 24 11">
      <defs>
        {gradients.map(({ id, previous, next, styles }) => {
          return (
            <linearGradient key={id} id={id} x1="5.5" y1="5.5" x2="12" y2="5.5" gradientUnits="userSpaceOnUse">
