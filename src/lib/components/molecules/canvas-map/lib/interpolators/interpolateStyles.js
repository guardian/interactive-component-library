...
export function interpolateStyles(firstStyle, secondStyle, interpolateColors, interpolateNumbers) {
...
export function interpolateFill(fillA, fillB, interpolateColors, interpolateNumbers) {
  const colorInterpolator = interpolateColors(fillA?.color ?? "transparent", fillB?.color ?? "transparent")
  const opacityInterpolator = interpolateNumbers(fillA?.opacity ?? 1, fillB?.opacity ?? 1)
...
export function interpolateStroke(strokeA, strokeB, interpolateColors, interpolateNumbers) {
  const colorInterpolator = interpolateColors(strokeA?.color ?? "transparent", strokeB?.color ?? "transparent")
  const opacityInterpolator = interpolateNumbers(strokeA?.opacity ?? 1, strokeB?.opacity ?? 1)
  const widthInterpolator = interpolateNumbers(strokeA?.width ?? 1, strokeB?.width ?? 1)
